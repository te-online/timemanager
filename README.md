...
Some end-to-end testing is done using [Cypress](https://www.cypress.io/). Tests currently cover basic features of the app and run in a headless Chrome browser on a containerized Nextcloud instance. For this repository tests are run manually before releases.
...
- Then in the `dev` directory run `podman-compose up`,
- Wait for the development Nextcloud to be ready,
- Then activate the app by running `podman exec -u www-data $(podman ps -qf "name=nextcloud-dev") sh -c 'php -f ./occ app:enable timemanager'`,
- In the UI, you need to add the user "alice" to the group "testgroup-1" (log in with admin/admin),
- Finally, open a new terminal in the `tests` directory, then run `npm start` and use Cypress' UI to start running the tests included in the spec file.
